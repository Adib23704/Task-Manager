...
export default function AuditLogTable({ logs }: Props) {
  if (logs.length === 0) {
    return <p className="text-gray-500 text-sm py-10 text-center">No audit logs recorded yet.</p>;
  }
        ...
        <tbody>
          {logs.map((log) => (
            <tr key={log.id} className="border-b border-gray-100 last:border-0">
              ...
              <td className="px-4 py-3 text-gray-700">{actionLabels[log.action] || log.action}</td>
              <td className="px-4 py-3 text-gray-500">{describeAction(log.action, log.details)}</td>
            </tr>
          ))}
        </tbody>
      </table>
